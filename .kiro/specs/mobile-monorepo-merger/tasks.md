...
- [ ] 1. Repair root workspace declaration and set up project infrastructure
  - [ ] 1.1 Update root `package.json` workspaces field to `["apps/*", "packages/*"]` and add `dev:mobile` script
  ...
  - [ ] 1.2 Create the SSoT violation scanner utility at `tests/utils/ssot-scanner.ts`
  ...
  - [ ] 1.3 Create the dependency isolation checker utility at `tests/utils/dependency-checker.ts`
  ...
  - [ ] 1.4 Create the file categorizer utility at `tests/utils/file-categorizer.ts`
  ...
  - [ ] 1.5 Set up vitest configuration and test infrastructure at project root
...
- [ ] 2. Implement property-based tests for core utilities
  - [ ]\* 2.1 Write property test for dependency isolation detection at `tests/pbt/dependency-isolation.test.ts`
  ...
  - [ ]\* 2.2 Write property test for SSoT violation detection at `tests/pbt/ssot-scanner.test.ts`
  ...
  - [ ]\* 2.3 Write property test for violation report completeness at `tests/pbt/report-completeness.test.ts`
  ...
  - [ ]\* 2.4 Write property test for file categorization correctness at `tests/pbt/file-categorizer.test.ts`
...
- [ ] 4. Execute backend purge on `apps/mobile`
  - [ ] 4.1 Run SSoT violation scan on current `apps/mobile` and record results
  ...
  - [ ] 4.2 Delete all legacy backend artifacts from `apps/mobile`
  ...
  - [ ] 4.3 Rewrite `apps/mobile/package.json` to remove legacy dependencies and add Expo/RN dependencies
...
- [ ] 5. Scaffold Expo + React Native project structure
  - [ ] 5.1 Create Expo Router file-based route structure under `apps/mobile/app/`
  ...
  - [ ] 5.2 Create core service and store files under `apps/mobile/src/`
  ...
  - [ ] 5.3 Create Expo configuration files at `apps/mobile/`
...
- [ ] 6. Map branding tokens to NativeWind v4 configuration
  - [ ] 6.1 Create `apps/mobile/tailwind.config.ts` with LibLog branding tokens
  ...
  - [ ] 6.2 Create `apps/mobile/global.css` with CSS variables for light and dark mode
  ...
  - [ ]\* 6.3 Write unit tests for branding token values at `tests/unit/branding-tokens.test.ts`
...
- [ ] 7. Rewrite documentation files
  - [ ] 7.1 Rewrite `apps/mobile/OVERVIEW.md` to reflect Mobile_App subordinate role
  ...
  - [ ] 7.2 Rewrite `apps/mobile/FEATURES.md` with canonical feature table
  ...
  - [ ]\* 7.3 Write unit tests for documentation content at `tests/unit/documentation.test.ts`
...
- [ ] 8. Generate Migration Audit Report
  - [ ] 8.1 Create `apps/mobile/MIGRATION_AUDIT.md` with all required sections
  ...
  - [ ]\* 8.2 Write unit tests for audit report structure at `tests/unit/audit-report.test.ts`
...
- [ ] 10. Verify build compatibility
  - [ ] 10.1 Run `npm install` at workspace root and verify both workspaces are recognized
  ...
  - [ ] 10.2 Run dependency isolation check post-install and update audit report
  ...
  - [ ] 10.3 Verify desktop dev server starts successfully and append build status to audit report
  ...
  - [ ] 10.4 Verify mobile Expo server starts successfully and finalize audit report
